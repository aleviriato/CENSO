CREATE OR REPLACE FORCE VIEW VW_RESUMO_CENSO_ATIV_BKP200619 AS
SELECT drive.etapa ,
       drive.PROCESSO ,
       drive.TOTAL,
  ...
  FROM (
         ...
         'FINALIZOU PRE-CADASTRO ONLINE' PROCESSO,
          COUNT(1) TOTAL
          ,round(round(100 * COUNT(1),2)/((select count(*)      from TB_USUARIO A1 WHERE EXISTS (SELECT 1 FROM TB_USUARIO_PERFIL UP WHERE UP.USUARIO_ID = A1.ID AND UP.PERFIL_ID = 2) )),2)       percentual_conclusao
          ,100-(round(round(100 * COUNT(1),2)/((select count(*) from TB_USUARIO A1 WHERE EXISTS (SELECT 1 FROM TB_USUARIO_PERFIL UP WHERE UP.USUARIO_ID = A1.ID AND UP.PERFIL_ID = 2) )),2))      percentual_remanescente
          FROM TB_USUARIO A
         ...
         'Acessou o Sistema' PROCESSO,
          COUNT(1) TOTAL
          ,round(round(100 * COUNT(1),2)/((select count(*)      from TB_USUARIO)),2)       percentual_conclusao
          ,100-(round(round(100 * COUNT(1),2)/((select count(*) from TB_USUARIO)),2))      percentual_remanescente
          FROM TB_USUARIO A
        ...
        UNION
          ...
          COUNT(1) TOTAL
          ,round(round(100 * COUNT(1),2)/((select count(*)      from TB_PESSOA_FISICA)),2)       percentual_conclusao
          ,100-(round(round(100 * COUNT(1),2)/((select count(*) from TB_PESSOA_FISICA)),2))      percentual_remanescente
            FROM TB_PESSOA_FISICA
           WHERE FLAG_RECADASTRAMENTO = 1
        ...
        SELECT 5 ETAPA
             ...
             , COUNT(1) TOTAL
             , round(round(100 * COUNT(1),2)/((select count(*)      from TB_USUARIO A1 WHERE EXISTS (SELECT 1 FROM TB_BENEFICIARIO   BE, TB_PESSOA_FISICA PF WHERE BE.COD_IDE_CLI = PF.COD_IDE_CLI AND PF.ID = A1.PESSOA_FISICA_ID AND BE.COD_TIPO_BENEFICIO <> '1'))),2)  percentual_conclusao
             , 100-(round(round(100 * COUNT(1),2)/((select count(*) from TB_USUARIO A2 WHERE EXISTS (SELECT 1 FROM TB_BENEFICIARIO   BE, TB_PESSOA_FISICA PF WHERE BE.COD_IDE_CLI = PF.COD_IDE_CLI AND PF.ID = A2.PESSOA_FISICA_ID AND BE.COD_TIPO_BENEFICIO <> '1'))),2)) percentual_remanescente
          FROM TB_USUARIO A
